...
if ($op == 'delete') {
  $id = $_GET['id'];
  $sql1 = "delete from profil_kades where id= '$id'";
...
if ($op == 'edit') {
  $id = $_GET['id'];
  $sqldef = "select * from profil_kades where id = '$id'";
...
if (isset($_POST['simpan'])) { //untuk create
  ...
  if ($id && $nama && $deskripsi && isset($_FILES['image']['name'])) {
    ...
    if (move_uploaded_file($image_tmp, $folder_tujuan)) {
      if ($op == 'edit') { //untuk update
        $sql1 = "update profil_kades set image='$image',nama='$nama', deskripsi='$deskripsi' where id = '$id'";
        $q1 = mysqli_query($conn, $sql1);
        if ($q1) {
        ...
        $sql1 = "insert into profil_kades(id,image,nama,deskripsi) values ('$id','$image','$nama','$deskripsi')";
        $q1 = mysqli_query($conn, $sql1);
        if ($q1) {
